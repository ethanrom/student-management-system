...
class IndexController extends BaseController{
	...
	private function login_user($username , $password_text, $rememberme = false){
		...
		$db->where("username", $username)->orWhere("useremail", $username);
		...
		if(!empty($user)){
			...
			$this->modeldata['userpass'] = $password_hash; //update the modeldata with the password hash
			if(password_verify($password_text,$password_hash)){
        		unset($user['userpass']); //Remove user password. No need to store it in the session
